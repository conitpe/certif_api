...
export class CertificadosService {
  ...
  async generateJsonLd(id: string) {
    ...
    try {
      recipientData = typeof certificado.recipient === 'string'
  ...
  const criterios = badge.criterios?.map(criterio => ({
    id: criterio.id,
    descripcion: criterio.descripcion
  })) || [];

  const habilidades = badge.habilidades?.map(habilidadBadge => ({
    ...
    const issuer = {
      ...
      type: "Issuer",
      name: badge.issuer.razon_social || "Nombre de la organización no disponible",
     ...
     return {
      ...
      "badge": {
        ...
        "criteria": {
        "narrative": "Este badge es otorgado por completar los criterios establecidos.",
        "detalles": criterios 
      },
      "skills": habilidades, 
      "issuer": issuer, 
      ...
      "issuedOn": certificado.issued_on ? certificado.issued_on.toISOString() : new Date().toISOString(),
  ...
  async generarCertificadoPDF(certificado: Certificado): Promise<Buffer> {
    if (!certificado) throw new NotFoundException(`El certificado con ID "${certificado.id}" no existe.`);
    if (!certificado.plantilla) throw new NotFoundException(`El certificado con ID "${certificado.id}" no tiene una plantilla asociada.`);
    ...
    if (plantilla.ruta_fondo) {
      ...
      if (imagePath.startsWith('http')) {
        ...
        imagePath = join(__dirname, '../../uploads', urlObj.pathname.split('/').pop());
      }

      if (!existsSync(imagePath)) {
        throw new NotFoundException(`La imagen de fondo no existe en la ruta: ${imagePath}`);
      }
    ...
    if (!beneficiario) {
      throw new NotFoundException(`No se encontró el beneficiario del certificado con ID "${certificado.id}".`);
    }
    ...
    if (!(fechaEmision instanceof Date) || isNaN(fechaEmision.getTime())) {
      throw new Error(`El campo fecha_emision no es una fecha válida: ${certificado.fecha_emision}`);
    }
    ...
    this.dibujarTextoDesdeHTML(page, nombreCompleto, nombreX, nombreY, plantilla.contenido_nombre, ancho, ajusteNombreX, ajusteNombreY);
    this.dibujarTextoDesdeHTML(page, fechaEmisionStr, fechaX, fechaY, plantilla.contenido_fecha_emision, ancho, ajusteFechaX, ajusteFechaY);
    ...
    page.drawImage(qrImage, {
      ...
      height: 100,
       ...
       if (certificado.badge?.ruta_imagen) {
        const badgeImageBytes = await this.descargarImagen(certificado.badge.ruta_imagen);
  
        const fileExtension = certificado.badge.ruta_imagen.split('.').pop()?.toLowerCase();
        ...
        if (fileExtension === 'jpg' || fileExtension === 'jpeg') {
          ...
          this.logger.error(`El archivo de imagen no es un formato válido: ${fileExtension}`);
          throw new Error(`El archivo de imagen no es un formato válido: ${fileExtension}`);
        }
  ...
  private dibujarTextoDesdeHTML(page, texto: string, x: number, y: number, html: string, ancho: number, ajusteX = 0, ajusteY = 0) {
    if (!html) return;

    const parsedHtml = parse(html).querySelector('span');
    if (!parsedHtml) return;

    const fontSize = parseInt(parsedHtml.getAttribute('style')?.match(/font-size:\s*(\d+)px/i)?.[1] || "16", 10);
    const colorHex = parsedHtml.getAttribute('style')?.match(/color:\s*(#[0-9A-Fa-f]+)/i)?.[1] || "#000000";
    const fontFamily = parsedHtml.getAttribute('style')?.match(/font-family:\s*([^;]+)/i)?.[1] || "Helvetica";
  ...
  async emitirCertificado(createCertificadoDto: CreateCertificadoDto): Promise<Certificado> {
    const { propietario, badge_id, plantilla_certificado_id, fecha_expiracion, metadata } = createCertificadoDto;
    ...
    try {
      ...
      if (propietario.organizacion_id) {
        organizacion = await queryRunner.manager.findOne(Organizacion, { where: { id: propietario.organizacion_id } });
        if (!organizacion) {
      ...
      // Buscar el badge
      const badge = await queryRunner.manager.findOne(Badge, { where: { id: badge_id } });
      if (!badge) {
        this.logger.warn(`Badge con ID "${badge_id}" no encontrado.`);
        throw new NotFoundException(`Badge con ID "${badge_id}" no encontrado.`);
      }
  
      // Buscar la plantilla asociada
      let plantilla = null;
      if (plantilla_certificado_id) {
        plantilla = await queryRunner.manager.findOne(PlantillaCertificado, { where: { id: plantilla_certificado_id } });
      } else {
  ...
  async update(id: string, updateCertificadoDto: UpdateCertificadoDto): Promise<Certificado> {
    ...
    try {
      ...
      const certificado = await queryRunner.manager.findOne(Certificado, { where: { id } });
      if (!certificado) {
      ...
      if (updateCertificadoDto.propietario) {
        let usuario = await queryRunner.manager.findOne(Usuario, {
          where: [{ email: updateCertificadoDto.propietario.email }, { dni: updateCertificadoDto.propietario.dni }],
        });
      ...
      await queryRunner.rollbackTransaction();
      this.logger.error(`Error al actualizar el certificado con ID ${id}: ${error.message}`);
  ...
  async delete(id: string): Promise<void> {
    ...
    try {  
      ...
      await queryRunner.rollbackTransaction();
      this.logger.error(`Error al eliminar el certificado con ID ${id}: ${error.message}`);
...
async findByUser(user_id: string): Promise<Certificado[]> {
  const usuario = await this.usuarioRepository.findOne({ where: { id: user_id } });
