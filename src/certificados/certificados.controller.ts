import { Controller, Get, Post, Body, Query,Put, Delete,  Param, BadRequestException,Res, NotFoundException} from '@nestjs/common';
...
export class CertificadosController {
  ...
  @ApiResponse({ status: 201, description: 'Certificado emitido correctamente.', type: Certificado })
...
@ApiResponse({ status: 200, type: [Certificado], description: 'Lista de certificados.' })
async findAll(@Query('badge_id') badge_id?: string) {
  ...
  async generarCertificado(@Param('id') id: string, @Res() res: Response) {
    ...
    const pdfBuffer = await this.certificadosService.generarCertificadoPDF(certificado);
    res.set({
  ...
  @Get('usuario/:user_id')
  async getCertificadosByUser(@Param('user_id') user_id: string): Promise<Certificado[]> {
 ...
 async getJsonLd(@Param('id') id: string) {
  ...
  @ApiResponse({
    ...
    schema: {
      properties: {
        items: { type: 'array', items: { $ref: '#/components/schemas/Certificado' } },
        total: { type: 'integer' },
      },
    },
  })
  ...
  @ApiOperation({ summary: 'Listar certificados por organización (paginado + búsqueda)' })
  @ApiResponse({
    status: 200,
    schema: {
      properties: {
  ...
  @ApiResponse({ status: 200, description: 'Certificado actualizado con éxito.' })
  @ApiResponse({ status: 404, description: 'Certificado no encontrado.' })
  async update(@Param('id') id: string, @Body() updateCertificadoDto: UpdateCertificadoDto) {
